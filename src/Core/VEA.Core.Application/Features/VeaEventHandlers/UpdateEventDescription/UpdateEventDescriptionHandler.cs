...
namespace VEA.Core.Application.Features.VeaEventHandlers.UpdateEventDescription;

internal class UpdateEventDescriptionHandler : ICommandHandler<UpdateEventDescriptionCommand, None>
{
    private readonly IVeaEventRepository _repository;
    private readonly IUnitOfWork _unitOfWork;

    internal UpdateEventDescriptionHandler(IVeaEventRepository repository, IUnitOfWork unitOfWork)
    {
    ...
    public async Task<Result<None>> HandleAsync(UpdateEventDescriptionCommand command)
    {
        var result = (await _repository.GetAsync(command.EventId))
            .Bind(veaEvent => veaEvent.UpdateDescription(command.Description))
